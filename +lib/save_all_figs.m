...
function subfolder_str = save_all_figs(FolderName,varargin)
time            = clock;
subfolder_str   = sprintf('%s%04d_%02d_%02d_%02d_%02d_%05d_figs/',FolderName,time(1),time(2),time(3),time(4),time(5),round(time(6)*1E3,3));
...
if nargin > 1
    out = varargin{1};
    save(fullfile(subfolder_str,'backup_data'),"out")
end
end